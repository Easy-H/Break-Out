...
public class QuestData : ScriptableObject
{
    ...
    public IQuestChecker GetQuestChecker() {
        switch (Type) { 
            case QuestType.killEnemy:
                ...
                return new KillEnemyQuestChecker(s[0], int.Parse(s[1]));
            case QuestType.killAllEnemy:
                return new KillAllEnemyQuestChecker(int.Parse(Value));
            default:
                return null;
        }
    }
    ...
    public static IQuestChecker GetQuestChecker(string value) {
        ...
        {
            case "killEnemy":
                return new KillEnemyQuestChecker(s[1], int.Parse(s[2]));
            case "killAllEnemy":
                return new KillAllEnemyQuestChecker(int.Parse(s[1]));
            case "killEnemyTotal":
